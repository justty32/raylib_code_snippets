...
//
// 河流走 tile 之間的邊，每條邊只被「兩個共享 tile 中的一個」儲存：
//   DIRECTIONS[0,1] (E/N) → tile 本身的 Tile.rivers slot 0/1
//   DIRECTIONS[2,3] (W/S) → 由鄰居儲存 slot 0/1（對應 dir - 2）
//
// rivers 存 2 × 8-bit 流量打包（uint32_t，只用低 16 bit）：
//   slot 0 = bits 0-7 (E), slot 1 = bits 8-15 (N)
//
...
// 渲染用：direction d 的邊由哪兩個 corner 連起來
// 四個角依序 NE=0, NW=1, SW=2, SE=3（順時針從右上開始）
extern const std::array<std::pair<int,int>, 4> EDGE_CORNERS;
...
[[nodiscard]] int  get_river_strength(const TileMap& map, const Coord& c, int direction) noexcept;
void               set_river_strength(TileMap& map, const Coord& c, int direction, int strength) noexcept;
void               add_river_flow    (TileMap& map, const Coord& c, int direction, int amount = 1) noexcept;
[[nodiscard]] bool has_river_edge    (const TileMap& map, const Coord& c, int direction) noexcept;
void               set_river_edge    (TileMap& map, const Coord& c, int direction, bool value = true) noexcept;

// Iterate: callback(origin_coord, direction 0..1, strength)
void iter_river_edges(const TileMap& map,
    std::function<void(const Coord&, int, int)> callback);
...
int generate_rivers(
    ...
    const std::vector<float>* temperature = nullptr,
